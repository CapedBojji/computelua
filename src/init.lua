...
function Dispatcher.Dispatch(self: Dispatcher, numThreads: number, batchSize: number, thread: string, useSerialDispatch: boolean?): Promise
...
function ComputeLua.CreateThread(actor: Actor, threadName: string, callback: (number, VariableBufferDataType) -> ())
	...
	actor:BindToMessageParallel(threadName, function(
		...
		for i = 1, batchSize do
			const id = (dispatchId - 1 * batchSize) + i
